...
$content_kk .= '
<div align="center"> 
<h3>LAPORAN REKAPITULASI HASIL LABORATORIUM PEMERIKSAAN KIMIA KLINIK</h3>
...
$qr_he = "SELECT tgl_hasil,no_rm,kode_lab,COUNT(no_rm) as norm,SUM(kimia_klinik) as kim FROM hasil WHERE kode_lab = '$kode_lab' AND tgl_hasil BETWEEN '$tgl_awal' AND '$tgl_akhir' AND kimia_klinik = '1' GROUP BY tgl_hasil,no_rm ORDER BY tgl_hasil,no_rm ASC";
...
while ($dt_he=mysqli_fetch_array($sq_he)) {
    ...
    while ($dt=mysqli_fetch_array($sqls)) {
        ...
        <td align="center">';
        if ($dsn != $tgl_hasil) {
            $content_kk .=TanggalIndo($tgl_hasil);
        }
        $dsn = $tgl_hasil;
...
//Rekap Hasil lab
$qr = "SELECT * FROM hasil WHERE kode_lab = '$kode_lab' AND tgl_hasil BETWEEN '$tgl_awal' AND '$tgl_akhir' AND kimia_klinik = '1'";
...
$pdf->writeHTML($content_kk, true, true, true, true, '');
...
$pdf->Output('LAPORAN HASIL LABORATORIUM PEMERIKSAAN KIMIA KLINIK '.TanggalIndo($tgl_awal).' '. TanggalIndo($tgl_akhir).'.pdf', 'D');
//============================================================+
// END OF FILE
//============================================================+
